...
where $f^*$ is the optimal cost and $f(n)$ is the cost of the shortest path
from the initial node to some goal node that contains a node $n$.  For many instances, a large
...
This is depicted in \refig{fig:plateau-0}. Naively, the search space of \astar could be considered to have a thin layer of final plateau surrounding the closed nodes with $f<f^*$ (left), which is no longer true. In reality, due to the curse of dimensionality, the search space has a large plateau of $f=f^*$ (right).
