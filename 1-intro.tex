...
\begin{enumerate}
 ...
 \item A new, thorough empirical analysis of the behavior of depth-based diversification
       (\refsec{sec:depth-based-evaluation}).
 \item We propose a framework for treating \astar as a sequence of satisficing searches on a set of $f$-cost plateaus (\refsec{sec:discussion}), and based on this framework, 
       \begin{enumerate}
        \item We identify the relationship between depth diversification and Type-GBFS
              \refsec{sec:depth-vs-types}.
        \item We demonstrated some of the theories in satisficing search literature
              can be effectively used for proving the completeness of \astar on infinite graph
              \refsec{sec:completeness-on-infinite-space}
        \item We propose and evaluate tie-breaking strategies which incorporates distance-to-go estimates \refsec{sec:distance-to-go}.
       \end{enumerate}
\end{enumerate}
