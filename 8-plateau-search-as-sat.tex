...
GBFS is solely guided by the heuristic function $h$, and heuristic errors in $h$ can easily misguided GBFS to spend all of its time in the wrong part of the search space -- GBFS can become trapped due to heuristic error and cannot recover from the wrong decision until expanding all nodes in that branch.
...
In our new framework of \astar as a search of satisficing searches, depth diversification can be viewed as the combination of (1) an implicit transformation of all edge costs within a single $f$-cost plateau to unit-costs, and (2) a pure type-based exploration within that plateau.
...
Depth diversification for tiebreaking in admissible A* has a different purpose and context from Type-GBFS for satisficing search,
...
Nevertheless, the close relationship between depth diversification for admissible A* and Type-GBFS for satisficing search shows that if we apply our framework of A* as a series of satisficing searches, we can directly use ideas which have been previously proposed for satisficing search within each $f$-cost plateau search.
