...
First, depth diversification  is focused on finding a satisficing plan \emph{within a single plateau} and on solving \emph{domains with zero-cost actions} % which is strictly harder than those without them. %worst-case complexity results for problem classes don't necessarily apply to specific instances, so using this ``strictly harder'' complexity result as justification is dangerous.
