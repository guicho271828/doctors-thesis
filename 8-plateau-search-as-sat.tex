...
\subsection{Depth Diversification in the New Perspective}

From our new view of \astar, the implementation of depth diversification can be seen as a variant of Type-based diversification approach \cite{xie14type} specifically tailored toward zero-cost domains.

\citeauthor{xie14type} proposed \emph{type based buckets}, an implementation of the OPEN list which stores the nodes in several buckets according to some set of key values called \emph{type}. In the paper, they proposed several types, for example $\brackets{1}$, $\brackets{g}$, $\brackets{h}$ or $\brackets{g,h}$. Upon each expansion, a single bucket is selected at random, and then a single node in the bucket is selected at random. For example, with type $\brackets{g,h}$, a node with $g=5$ and $h=3$ is put into a bucket of $\brackets{5,3}$. This mechanism diversifies the search so that it tries to expand the nodes with the various distance from the initial state and the various distance from the goal state.

They then proposed Type-based GBFS, which alternates the expansion between the normal GBFS with $[h,\fifo]$ sorting criteria and the type based buckets. The aim of alternating the normal GBFS expansion and type-based expansion in this framework is to reduce the weakness of GBFS: Original GBFS is solely guided by the heuristic function $h$ and can be easily misguided by the heuristic errors in $h$ into the wrong direction of the search space, and it cannot recover from the wrong decision until expanding all nodes in that branch. 
Due to this characteristics, on infinite graphs, GBFS is not complete \cite{Valenzano2016}
because it can be misdirected by the heuristic guidance forever.
In contrast, in type-based GBFS, the alternation to the type buckets introduces a certain degree of exploratory behavior by random restarts to low $g$ and high $h$, reducing the negative impact of the heuristic error.
As a result, type-based GBFS is probabilistically complete on infinite graphs \cite{Valenzano2016}.

Type-based GBFS was primarily evaluated in the context of satisficing search with no plan quality consideration.
This leads to the adaptation of unit-cost domain model: All action costs are ignored and replaced with unit costs in their experiments in order to boost the coverage. This technique is also used in the first iteration of LAMA2011 \cite{richter2011lama}. As a result, its performance is solely evaluated by the coverage.

% (i.e., finding either a goal node or exhausting the plateau)

If we adopt the new view to \astar, depth diversification can be seen as using the type buckets with depth as a key, similarly replacing all costs with the unit costs, but with one essential difference: All these diversification, depth measurement and cost replacement are confined within a particular plateau.
 
The notion of \emph{depth} counts the number of zero-cost actions, which does not change the $f$ value and $h$
value, on the path to the current node from the plateau entrance, which is an interface to another plateau. Thus, the
depth considers the problem of finding an exit from a particular plateau as a unit-cost satisficing search problem
-- the depth is analogous to a $g$-value calculated with unit costs and restricted to a particular plateau.
...
Depth diversification for tiebreaking has a different purpose and context from Type-GBFS,
and is novel in the following aspects.
First, it is focused on finding a satisficing plan \emph{within plateau} and on solving \emph{domains with zero-cost actions} which is strictly harder than those without them. Also, the role is limited to each plateau with the same $f$ and $h$ because nodes are put in the depth buckets only after being sorted by $f$ and $h$, while type buckets are global --- type buckets have no preceding sorting criteria, and the entire open nodes are stored in these buckets.
This behavior of depth diversification allows the search to find a solution efficiently, without losing optimality.
Depth diversification proved the importance of tiebreaking in the context of cost-optimal planning and achieves a significant performance improvement.

Despite all this, however, the new view is so powerful that it unifies the two algorithms that have been developed separately for different purposes.
