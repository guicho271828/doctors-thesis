...
Type-based GBFS was primarily evaluated in the context of satisficing search with no consideration of plan quality, and performance is solely evaluated according to coverage.
...
First, it is focused on finding a satisficing plan \emph{within a single plateau} and on solving \emph{domains with zero-cost actions} which is strictly harder than those without them. Due to this purpose, depth diversification is applied after the sorting by $h$.
