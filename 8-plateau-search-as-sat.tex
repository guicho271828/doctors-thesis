...
This is somewhat similar to the standard approach to model-based sequential / temporal planning using SAT/IP/CP solvers \cite{kautz1992planning,van2005optiplan}.
...
\begin{proof}
 \refalgo{alg:astar-sat} requires that the algorithm must prove the UNSAT-isfiability (that there is no solution) of all non-final plateaus, $\plateau{f}$ ($\forall f<f^*$), so if any of them are infinite, \astar does not terminate.
\end{proof}
...
\begin{theo}[\citeauthor{Valenzano2016} \citeyear{Valenzano2016}]
 ...
 approaches to 1 when the number of expansion $t$ approaches to $\infty$.
\end{theo}
...
Next, we show the completeness of our iteration-based depth diversification \refalgo{}.
