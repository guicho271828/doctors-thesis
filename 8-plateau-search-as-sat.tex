...
As another example of the usefulness of the framework, we show that we can prove the completeness of \astar on infinite graph depending on the tie-breaking criteria.
As long as the search graph is finite, \astar is complete regardless of the tie-breaking strategy.
However, if the search graph is infinite (i.e. the number of nodes in the graph has no upper bound), the completeness of the algorithm could be affected.
In order to analyze \astar, we examine several cases depending on which plateau is infinite.
...
\begin{proof}
 \refalgo{alg:astar-sat} requires that the algorithm must prove the UNSAT-isfiability (that there is no solution) of non-final plateaus, $\plateau{f}$ ($\forall f<f^*$). If any of them are infinite, it does not terminate.
\end{proof}

So we next examine the case where all of $\plateau{f}$ for $\forall f<f^*$ are finite.
...
Thus we can carry theorems from the existing literature.
...
\begin{theo}
 \astar with Random Order tiebreaking $[f,\ro]$ is probabilistically complete on an infinite graph satisfying the conditions in Proposition 2.
\end{theo}
...
\begin{propo}
 \astar with FIFO tiebreaking $[f,\fifo]$ is complete on an infinite graph when \emph{(1)} it satisfies the conditions in Proposition 2, \emph{(2)} the branching factor is finite and \emph{(3)} the solution length is finite. (Trivially shown by the completeness of Breadth-first search on $\plateau{f^*}$.)
\end{propo}
...
While these are trivial proofs, we next show the completeness of our iteration-based depth diversification.
