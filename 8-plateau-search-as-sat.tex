...
\begin{propo}
 ...
 $\plateau{f}$ is finite  $\forall f<f^*$,
 solution length is finite and
 the branching factor is finite in $\plateau{f^*}$ below the solution depth.
\end{propo}

LIFO tie-breaking performs a depth-first search with duplicate detection (DFS-dup) on $\plateau{f^*}$.
