...
% 
In the following two subsections, as well as in the next section, we show that (1) it provides a better understanding of depth-diversification (\refsec{sec:depth-vs-types}), (2) it allows us to prove the completeness of \astar on infinite graph depending on the tie-breaking methods (\refsec{sec:completeness-on-infinite-space}), and (3) the idea allows us to improve the performance of \astar on Zerocost domains (\refsec{sec:distance-to-go}).
...
% Given that, the last condition we should examine is whether $\plateau{f^*}$ is infinite.

% \begin{propo} % I don't think this is a formal proposition because ``completeness depends on'' is not clearly defined 
% Given a set of finite $\plateau{f}$ for $\forall f<f^*$ and an infinite $\plateau{f^*}$,
...
% I think what you're  really trying to do with the prop is encapsulate the conditions so you can refer to it later, but it seems that directly stating these  assumptions isn't muchlonger than saying ``the conditions in Proposition X''. So...
...
depends on the completeness of satisficing search algorithm
$[\text{criterion}_2, \ldots \text{criterion}_k]$ on $\plateau{f^*}$.
For the standard tie-breakig criteria, we can apply previously known results.
% , such as \cite{Valenzano2016}.
% Reiterating \cite{Valenzano2016}, 
...
\begin{corollary}
 ...
 when  $\plateau{f}$ is finite  $\forall f<f^*$, and $\plateau{f^*}$ is infinite. %stating the conditions is not much longer than referencing conditions
 %satisfying the conditions in Proposition 2.
\end{corollary}

\begin{proof}
  $[f,\ro]$ is an instance of \astar using $[\ro]$ as a satisficing algorithm for plateau-search. Since $[\ro]$ is a special case of $\varepsilon$-greedy node selection with $\varepsilon=1$, $[f,\ro]$ with an infinite $\plateau{f^*}$ is also probabilistically complete.
\end{proof}

Breadth-first search is complete when the graph has finite branch factor, and solution length is finite.
Since FIFO tiebreaking $[f,\fifo]$ applies breadth-first search to $\plateau{f^*}$,  it follows that

\begin{propo}
% \astar with FIFO tiebreaking $[f,\fifo]$ is complete on an infinite graph when \emph{(1)} it satisfies the conditions in Proposition 2, \emph{(2)} the branching factor is finite and \emph{(3)} the solution length is finite. (Trivially follows from the completeness of Breadth-first search on $\plateau{f^*}$.)
...
LIFO tie-breaking performs depth-first search on $\plateau{f^*}$. Therefore, the completeness of \astar using LIFO tiebreaking depends on the search space -- in general, LIFO tie-breaking is not complete.
...
% ``solution length is finite'' -- does the existence of a ``solution'' implies finite length?

% finite branch factor and finite max depth --> finite space,  which is out of scope
...
\begin{propo} %corollary? (because dfs incompleteness is known under these conditions)
...
\begin{proof}
 ...
 it is possible for the $[f,\lifo]$ to be trapped in an infinitely large region (e.g., if $[f,\lifo]$ starts exploring an infinitely large region where all edge costs have zero cost,  depth-first exploration of $plateau(f)$ never terminates). 
% On a graph with (4) a finite maximum depth,
% it still has a possibility to search the infinitely large region in a subtree
% which does not contain the solution.
% not sure what was intended by ``finite maximum depth...''  here -- ``infinitely large region in a subtree'' where branch factor is finite must have infinite depth.
\end{proof}

Next, we show the completeness of our iteration-based depth diversification \refalgo{alg:depth}.


\begin{theo}
 On a graph with finite branching factor,
 if $\plateau{f}$ is finite $\forall f<f^*$
 and $\plateau{f^*}$ is infinite,
%  \emph{(1)} it satisfies the conditions in Proposition 2,
% \emph{(2)} the branching factor is finite and 
% \emph{(3)} the solution length is finite % ``solution'' by definition must be  finite length?
