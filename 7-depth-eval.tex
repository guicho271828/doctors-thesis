...
default tiebreaking criterion.

Experiments are conducted on \textbf{1104 standard IPC benchmark
instances} from 35 domains  and \textbf{620 zerocost instances} from 28 domains (see \refsec{sec:eval-common-strategies} and \refsec{sec:zerocost-domains} for full lists of these domains).  Each experiment uses
The Fast Downward planner using A* search and either the \lmcut heuristic or \mands heuristic.  For \mands,
we used the currently recommended settings\footnote{http://XXXfast-downward.org/XXX, as of 2016-5-11} 
...
histograms of the depths of search nodes expanded by several tiebreaking
...
In addition, we  added some instrumentation to the strategies which do not use depth-based tiebreaking ($[f,h,\fifo]$, $[f,h,\lifo]$, $[f,h,\ro]$) so that as these strategies run, the depth of each of the expanded methods is computed (although they do not affect the search behavior).
Note that this instrumentation, which addes some runtime overhead, was \emph{not}
