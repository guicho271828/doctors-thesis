...
as the first-level tie-breaking and either \fifo, \lifo or \ro (random order) as the default tie-breaking criterion
...
\begin{figure}[htbp]
 ...
 % 
 \caption{Comparison of the node evaluation ratio (node/sec) between standard tie-breaking ($y$-axis) and
 depth-based tie-breaking ($x$-axis) on \lmcut and \mands heuristics. On \mands, compared to \lmcut, node evaluation rate more often becomes
 twice as slower when depth is enabled. This is because the node evaluation of \mands is an order of
 magnitude faster than \lmcut, and the overhead of managing depth-based tie-breaking queue becomes significant. Same results were obtained with \lifo and \ro default tie-breaking. }
 % 
