...
\begin{figure}[htb]
 ...
 by \mands heuristics, with vs without depth-based tiebreaking.
...
Moreover, the results suggest that the distribution almost follows
$D-d$, a theoretial formula which was made under the simplified
assumption that the plateau forms a tree of fixed width.
...
\includegraphics{img/depth/miconic-up/p79.pdf}
 \caption{(Page 1/3) Number of nodes ($y$-axis) expanded per depth ($x$-axis) in
...
\includegraphics{img/depth/rovers-fuel/p07.pdf}
 \caption{(Page 2/3) Number of nodes ($y$-axis) expanded per depth ($x$-axis) in
 the final plateau with different tiebreakings. Both axes are in logarithmic scale.
 }
...
\includegraphics{img/depth/zenotravel-fuel/p07.pdf}
 \caption{(Page 3/3) Number of nodes ($y$-axis) expanded per depth ($x$-axis) in
 the final plateau with different tiebreakings. Both axes are in logarithmic scale.
 }
