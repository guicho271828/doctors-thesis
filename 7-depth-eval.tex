...
Thanks to the unbiased search, depth-based strategy has a better chance of improving $h$ values, finding a node in $\plateau{f^*,0}$ more quickly.
This shows that the depth can also enhance the search in non-final plateaus to find the nodes in the next plateau.
Similar phenomena were observed in several other instances and domains (\refig{fig:depth-histogram5}). To list a few, they are the following: depot-fuel, driverlog-fuel, zenotravel-fuel, floortile-ink, mprime-succumb, storage-lift.
