...
the first state which entered the plateau, along the path from the
initial state. These notions are depicted in
...
It's search behavior is not biased toward any particular direction and
...
\begin{itemize}
 \item If $f$-value is unchanged after operator application, $g$ value
       should have been increased due to a positive
       cost edge and thus $h$ value should have been
       decreased if the evaluated node is new.
 \item If $h$-value is unchanged and $g$ is increased due to a positive
       cost edge, then $f$ is also increased.
\end{itemize}
...
prune the redundant nodes. Note that redunduncy cause a biased 
...
group $A$ by giving a $\times 4$ larger chance of expansion than $b$,
$c$ or $d$.
