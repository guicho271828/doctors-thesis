...
There is one Depth-Diversified Node Selector instance per each plateau (either being $\plateau{f}$ or $\plateau{f,h}$) which is responsible for this behavior.
...
Each plateau is now represented by a selector instance.
Then we simply call $\text{pop}(\text{selector})$ method on this instance which returns a node.
Each instance holds
an index $d_c$,
