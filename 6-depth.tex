...
In the diversification, although the random selection from the depth buckets is possible, our implementation performs a deterministic, round-robin sampling from the available depth buckets as described above.
...
Next, we assume that no depth bucket exhausts due to the expansion (\emph{no-exhaust assumption}).
...
happened only $D-d$ times (since $w>2$, $w(D-d+1)>2(D-d)+2>D-d$).
...
For example, in \fifo, all nodes with $d<D$ are expanded while depth $d=D$ can take arbitrary number of expansions $e \in [0, Dw^D]$. Also, in \lifo, for some $k\in [1,D]$, there can be a situation where all depths $d \in [0,k-1]$ gets only 1 expansion while all nodes in depths $d \in [k,D]$ are expanded. Such an imbalance during the search causes the pathological behavior described above.
