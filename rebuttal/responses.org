...
It's true that if all nodes in the entire graph are expanded, there are surely more nodes with larger depth.
However, with a reasonably good heuristic, A* expands only a small fraction of nodes in the search space, an the nodes expanded by A* during the search process are biased to the shallower region 
...
** [3.3-INPROGRESS] 3) This applies also to the summary of the authors earlier conference paper.
...
The paragraphs at the start of Section 5 define the set of 28 Zerocost
...
Table [Original:7.2, Revised:12.3] (Zerocost instances) and Table [Original:7.4, Revised:12.5] (IPC instances).
Qualitiatively, the results for [f,h,lifo] are similar to that of [f,h,fifo] -- Zerocost differences are "harder" than their corresponding IPC instances.
