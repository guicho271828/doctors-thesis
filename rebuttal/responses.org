...
comparing the node evaluation ratio, because this is essentially 1-dimensional data and the domain characteristics is not important
...
We acknowledge that these figures are crowded it's hard to distinguish
...
** [2.2] 2) The description in the early part of the paper (Sections 1, 3, 4, 5) somewhat convey the false impression that there has been no previous recognition of the challenge that plateaus can create for A* search
...
we added the statement that the huge plateaus induced in the final plateau are instances of $g$-value plateaus described by Benton et al 2010.
