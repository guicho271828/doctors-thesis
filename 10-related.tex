...
% 
A slack means a constant amount of error from the optimal value. With a slack, one can model a situation where the
goal is to optimize the first preference, but the difference up to some amount is ignored and ties are broken by
the second preference.
% 
One example planning problem with such lexicographic preferences with slack would be something like this:
Its first optimization objective is the amount of fuel usage, allowing a slack up to 5 liters.
...
% 
Since a slack allows multiple values (e.g. 100 and 105) to have the same preference, it is supposed to introduce larger plateaus.
Applying our techniques to the problems with slack is an avenue of future work.
