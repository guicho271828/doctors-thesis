...
Existing empirical analyses mainly focus on the performance difference caused by using a plan length heuristic for inadmissible heuristic search (further discussed in \refsec{sec:distance-to-go}), not the performance difference caused by the target cost function in cost-optimal search.
