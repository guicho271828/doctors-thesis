...
One characteristics of \pddl{Openstacks} is the presence of many  actions which have zero cost becuase they do not increase the number of stacks. These zero-cost actions creates the problem depicted in \refig{fig:plateau-1}.
...
This creates huge heuristic plateaus that share the same $h$-value, and the standard $h$-based tiebreaking criterion can not provide informative guidance for search within the plateau $f=f^*$.\todo{why mention $f=f^*$ specifically here, instead of ``final plateau'' (f*,0) or just in general all plateaus?}
...
\begin{table}[htbp]
 ...
 \caption{
 Assessment of the relative difficulty of zero-cost domains vs. their corresponding, standard domain, for the standard $[f,h,lifo]$ and $[f,h,fifo]$ tiebreaking strategies.
...
computation.\todo{this is talking about the distance-to-go, but seems better to skip this paragraph at this point int he paper, and not mention 2 options ye}
