...
\begin{figure}[htbp]
 ...
 \caption{Search space of \astar and its contour according to $h$. (\textbf{Right}) In domains with only positive-cost actions, $h$-based tiebreaking provides meaningful guidance. (\textbf{Left}) In domains with 0-cost actions, applying an action may not increase the cost of the path and the region with $h=0$ could be quite large. Thus, $h$-based tiebreaking fails to provide meaningful guidance in this space.
  }
 \label{fig:plateau-1}
\end{figure}
...
In some domains (specifically, \pddl{blocks}, \pddl{freecell}, \pddl{pipesworld-notankage}, \pddl{miconic}), the set of zerocost domain instances has fewer instances than the original domain. % \pddl{blocks} original 35 vs 20, \pddl{freecell} 80 vs 20, pipesnt 50 vs 20, \pddl{miconic} 150 vs 30
...
instances by picking instances  p05, p10, ... p150.
